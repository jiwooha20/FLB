...
tokenizer = AutoTokenizer.from_pretrained("/home/de-hallucination/ZCD/VCD/experiments/checkpoints/llava-v1.5-7b", use_fast=False)
...
def sample(
    ...
    **model_kwargs,
    ...
    # ==== [Begin] Added by Jiwoo Ha (2025-12-01) ===============================
    use_flb = model_kwargs.get("use_flb") 
    flb_gamma = model_kwargs.get("flb_gamma", 0.3)
    flb_lambda = model_kwargs.get("flb_lambda", 0.05)

    step=0

    image_only_logits = None
    # ==== [End] Added by Jiwoo Ha (2025-12-01) ===============================
    ...
    while True:
        ...
        # ==== [Begin] Added by Jiwoo Ha (2025-12-01) ===============================
        if use_flb and step == 0:
            image_only_logits = next_token_logits.detach().clone()
        # ==== [End] Added by Jiwoo Ha (2025-12-01) ===============================
        ...
        # ==== [Begin] Added by Jiwoo Ha (2025-12-01) ===============================
        if use_flb and image_only_logits is not None:
            img_only_weight_t = flb_gamma * (1 - torch.exp(torch.tensor(-flb_lambda * step)))

            diffs = next_token_logits + img_only_weight_t * image_only_logits
